...
function openAddPropertyModal(condominioId = null) {
  const modal = document.getElementById('add-property-modal');
  if (modal) {
    modal.style.display = 'flex';
  }
  
  // Poblar el select de condominios
  const condoSelect = document.getElementById('property-condominio');
  if (condoSelect) {
    condoSelect.innerHTML = '<option value="">Selecciona un condominio</option>';
    
    condominiosCache.forEach(condo => {
      const option = document.createElement('option');
      option.value = condo.id;
      option.textContent = condo.name;
      if (condominioId === condo.id) {
...
async function guardarPropiedad(event) {
  ...
  const condominioId = document.getElementById('property-condominio').value;
  ...
  if (!condominioId || !type || !name) {
    showFeedback('Completa los campos requeridos (Condominio, Tipo, Nombre)', 'error');
    return;
  }
  
  // Convertir imagen a base64 si existe
  let imagenBase64 = DEFAULT_PROPERTY_IMAGE;
  if (imageFile) {
...
function cargarReservas(data, amenities = []) {
  const reservasContainer = document.getElementById('reservas-container');

  if (!reservasContainer) return;

  // Mostrar solo reservas aprobadas (el sistema las aprueba automáticamente)
  reservasCache = (data || []).filter(r => r.estado === 'aprobada');

  // Remover antiguo contenedor de filtro padre si existe
  const existingWrapper = document.getElementById('reservas-parent-filter-wrapper');
  if (existingWrapper) existingWrapper.remove();

  // Construir filtro padre compacto (barra horizontal) + panel desplegable
  const parentHtml = `
    <div id="reservas-parent-filter-wrapper">
      <div class="reservas-parent-filter">
        <div class="reservas-parent-inner">
          <div class="reservas-parent-left">
            <strong>Reservas aprobadas</strong>
            <span id="reservas-count" class="reservas-count">${reservasCache.length}</span>
          </div>
          <div class="reservas-parent-actions">
            <button id="reservas-toggle-filters" class="filter-toggle-btn">Filtros <i class="fas fa-chevron-down"></i></button>
          </div>
        </div>
        <div id="reservas-filter-panel" class="reservas-filter-panel" style="display:none;">
          <div class="reservas-filter-row">
            <div class="filter-group">
              <label>Rango de fechas</label>
              <div class="date-range">
                <input type="date" id="reserva-start-date" class="modal-input" />
                <span class="date-sep">—</span>
                <input type="date" id="reserva-end-date" class="modal-input" />
              </div>
              <div class="quick-buttons">
                <button type="button" class="filter-quick-btn" id="reserva-quick-today">Hoy</button>
                <button type="button" class="filter-quick-btn" id="reserva-quick-week">Esta semana</button>
                <button type="button" class="filter-quick-btn" id="reserva-quick-month">Mes</button>
              </div>
            </div>
            <div class="filter-group">
              <label>Amenidad</label>
              <select id="reservas-amenity-filter" class="modal-input"><option value="">Todas</option></select>
            </div>
            <div class="filter-actions">
              <button class="admin-action-btn" id="reservas-apply-filters">Aplicar</button>
              <button class="admin-secondary-btn" id="reservas-clear-filters">Limpiar</button>
            </div>
          </div>
        </div>
      </div>
  ...
  tmp.innerHTML = parentHtml;
  const parentNode = tmp.firstElementChild;

  // Insertar antes del contenedor de reservas
  reservasContainer.parentNode.insertBefore(parentNode, reservasContainer);

  // Poblar select de amenidades
  const amenitySelect = document.getElementById('reservas-amenity-filter');
  if (amenitySelect && Array.isArray(amenities)) {
    amenities.forEach(a => {
  ...
  // Bind eventos
  const toggleBtn = document.getElementById('reservas-toggle-filters');
  const panel = document.getElementById('reservas-filter-panel');
  const applyBtn = document.getElementById('reservas-apply-filters');
  const clearBtn = document.getElementById('reservas-clear-filters');
  const quickToday = document.getElementById('reserva-quick-today');
  const quickWeek = document.getElementById('reserva-quick-week');
  const quickMonth = document.getElementById('reserva-quick-month');

  if (toggleBtn && panel) {
    toggleBtn.addEventListener('click', () => {
      const isOpen = panel.style.display === 'block';
      panel.style.display = isOpen ? 'none' : 'block';
      toggleBtn.querySelector('i')?.classList.toggle('open', !isOpen);
    });
  }

  if (quickToday) quickToday.addEventListener('click', () => { setRangeToday(); applyReservasFilters(); });
  if (quickWeek) quickWeek.addEventListener('click', () => { setRangeWeek(); applyReservasFilters(); });
  if (quickMonth) quickMonth.addEventListener('click', () => { setRangeMonth(); applyReservasFilters(); });

  if (applyBtn) applyBtn.addEventListener('click', applyReservasFilters);
  if (clearBtn) clearBtn.addEventListener('click', () => {
    document.getElementById('reserva-start-date').value = '';
    document.getElementById('reserva-end-date').value = '';
    if (amenitySelect) amenitySelect.value = '';
    applyReservasFilters();
  });

  // Render inicial: aplicamos filtros (por defecto solo aprobadas)
  applyReservasFilters();
}
...
function setRangeToday() {
  ...
  const start = document.getElementById('reserva-start-date');
  const end = document.getElementById('reserva-end-date');
  if (start) start.value = val;
  if (end) end.value = val;
}
...
function applyReservasFilters() {
  const start = document.getElementById('reserva-start-date')?.value || '';
  const end = document.getElementById('reserva-end-date')?.value || '';
  const amenity = document.getElementById('reservas-amenity-filter')?.value || '';

  let filtered = reservasCache.slice();

  if (start) filtered = filtered.filter(r => r.fecha >= start);
  if (end) filtered = filtered.filter(r => r.fecha <= end);
  if (amenity) filtered = filtered.filter(r => r.amenityId === amenity || r.amenidad === amenity);

  renderReservasList(filtered);
...
function renderReservasList(list) {
  ...
  if (!list.length) {
    reservasContainer.innerHTML = '<div class="admin-empty-state">No hay reservas que coincidan con los filtros seleccionados.</div>';
    return;
  }
    ...
    reservasContainer.innerHTML += `
      <div class="card">
        <div class="card-info">
          ...
          ${reserva.siguienteEstado ? `
            <button class="admin-action-btn" onclick="actualizarEstadoReserva('${reserva.id}', '${reserva.siguienteEstado}')">Marcar como ${reserva.siguienteEstadoLabel}</button>
          ` : ''}
        </div>
      </div>
    `;
...
function changeTab(tabName) {
  ...
  // Asegurar que el filtro padre de reservas solo esté visible cuando estamos en la pestaña 'reservas'
  const existingReservasWrapper = document.getElementById('reservas-parent-filter-wrapper');
  if (existingReservasWrapper && tabName !== 'reservas') {
    existingReservasWrapper.remove();
  }
  ...
  if (tabName === 'reservas') {
    ...
    Promise.all([cargarReservasDesdeApi(), cargarAmenidadesDesdeApi()])
      .then(([reservations, amenities]) => {
        cargarReservas(reservations, amenities);
      })
      .catch((error) => {
